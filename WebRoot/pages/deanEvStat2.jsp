...
<%
	...
	if (!isAdmin)
	{
		Purview = jdb.getIntBySql(0, "select top 1 Purview from ObjRight where ObjType=10 and ObjParam='deanEvStat2' and GroupID in (" +
			user.MemberGroupIDs + ") order by Purview desc");
	}
	SysApp sysApp = new SysApp(jdb);
//@@##636:服务器启动代码(归档用注释,切勿删改)
...
var sys = {EName:"<%=user.EMemberName%>", CName:"<%=user.CMemberName%>", PhotoID:<%=user.PhotoID%>, PageName:"deanEvStat2", Role:<%=Purview%>};
var book;
...
//@@##637:客户端自定义代码(归档用注释,切勿删改)
function ClickEVTask(obj, item) {
	
	if (book.Page == undefined)
	{
		book.Page = new $.jcom.DBGrid(book.getDocObj("Page")[0], "../fvs/CPB_EvaluateTask_deanEvStat2.jsp"
