	...
	, String OpenId, String NickName)  throws Exception
{
	...
	String sql = "";
	ResultSet rs;
	sql = "select * from Member where SPassword='" + MD5Pass +"' and (EName ='" + UserName + "' or CName='" + UserName +
...
<%
	...
	if (!UserName.equals(""))
	{
		...
		if (err.equals(""))
		{
			err = CheckLogin(1, UserName, Password, response, OpenId, NickName);
			if (err.equals("OK"))
				loginErrors = -1;
		}
...
<head>
	...
	<script type="text/javascript" src="../biapages/local.js"></script>
<style type="text/css">
#DocFrame{
background: #e9ebee url(bia.jpg) no-repeat;
...
window.onload = function()
{
	...
	sysItem = getLocalItem("BIMain");
	t.html("<div align=center style='color:white;font:normal normal normal 46px 微软雅黑'>" + sysItem.item + "</div>");
	var s = book.getDocObj("SignBox");
	var fields = [{CName:"用户名", EName: "UserName", nRequired:1},
...
function LoginResult(result)
{
	if (result == "OK")
	{
		location.href = "../biapages/" + sysItem.Code + ".jsp";
		return;
	}
	loginErrors ++;
	alert(result);
	var fields = book.form.getFields();
	if ((loginErrors > CodeErrs) && (fields.length == 4))
	{
		fields[fields.length] = oCheck;
