...
// ServerChapterPageStartCode
	...
	String CodeName="BIReport";
	int Purview = 16;
	if (!isAdmin)
	{
		Purview = jdb.getIntBySql(0, "select top 1 Purview from ObjRight where ObjType=10 and ObjParam='BIReport' and GroupID in (" +
...
var sys = {EName:"<%=user.EMemberName%>", CName:"<%=user.CMemberName%>", nType:<%=loginType%>, PhotoID:<%=user.PhotoID%>, PageName:"BIReport", Role:<%=Purview%>};
...
//@@##201:客户端加载时(归档用注释,切勿删改)
//
//(归档用注释,切勿删改)ClientStartCode End##@@
};

//@@##202:客户端自定义代码(归档用注释,切勿删改)
//
