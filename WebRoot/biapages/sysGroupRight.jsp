...
// ServerChapterPageStartCode
	...
	String CodeName="sysGroupRight";
	int Purview = 16;
	if (!isAdmin)
	{
		Purview = jdb.getIntBySql(0, "select top 1 Purview from ObjRight where ObjType=10 and ObjParam='sysGroupRight' and GroupID in (" +
...
<style id="css_146" type="text/css">
.pcbody{
...
var sys = {EName:"<%=user.EMemberName%>", CName:"<%=user.CMemberName%>", nType:<%=loginType%>, PhotoID:<%=user.PhotoID%>, PageName:"sysGroupRight", Role:<%=Purview%>};
...
window.onload = function()
{
	...
	var Tools = [
		{item:"新增分组", img:"", action:NewGroup, info:"", Attrib:2, Param:0},
		{item:"修改分组", img:"", action:EditGroup, info:"", Attrib:2, Param:0},
		{item:"删除分组", img:"", action:DelGroup, info:"", Attrib:2, Param:0},
		{item:"新增成员", img:"", action:NewMember, info:"", Attrib:2, Param:0},
		{item:"删除成员", img:"", action:DelMember, info:"", Attrib:2, Param:0},
		{item:"保存设置", img:"", action:SaveRight, info:"", Attrib:2, Param:0}
			];
	book.setTool(Tools);
//@@##127:客户端加载时(归档用注释,切勿删改)
//
	var catlog = book.getChild("catalog");
	var items = $.extend(true, [], catlog);
	
	book.Page = new GroupRightPage($("#Page")[0], items);
...
//		 	PrepareData(Chapters);
		 	...
		 	grid.TitleBar("<h1 align=center>页面角色设置</ht>");
	...
	function ClickGroup(obj, item)
	{
		...
		{
			grid.TitleBar("<h1 align=center>" + item.item + "页面角色设置</ht>");
